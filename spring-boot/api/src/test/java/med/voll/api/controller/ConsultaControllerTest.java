...
class ConsultaControllerTest {
    ...
    @Test
    @DisplayName("Deveria devolver código HTTP 400 quando formações estão invalidas")
    @WithMockUser
    ...
    @Test
    @DisplayName("Deveria devolver código HTTP 200 quando formações estão validas")
    @WithMockUser
    void agendarCenario2() throws Exception {
        ...
        MockHttpServletResponse response = mvc.perform(
                post("/consultas")
                        ...
                        .content(dadosAgendamentoConsultaJson.write( new DadosAgendamentoConsulta(
                                2l, 5l,
                                data, especialidade)).getJson()))
                .andReturn().getResponse();

        assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());

        var jsonEsperado = dadosDetalhamentoConsultaJson.write(
                dadosDetalhamentoConsulta).getJson();
